...
import { validateCpfBatch } from "@/lib/cpf";
import type { CpfValidationResult } from "@/lib/cpf";

export function ValidatorPanel() {
  const [input, setInput] = useState("");
  const [results, setResults] = useState<
    { value: string; result: CpfValidationResult }[] | null
  >(null);
  ...
  function handleValidate() {
    ...
    const validated = validateCpfBatch(input);
    setResults(
  ...
  return (
    <section className="space-y-4 rounded-2xl border bg-card p-5 shadow-xs sm:p-6">
      <div className="space-y-2">
        <Label htmlFor="validator-input">CPFs para validar</Label>
        <Textarea
          id="validator-input"
        ...
        <p className="text-xs text-muted-foreground">
          Separe múltiplos CPFs por linha, vírgula ou ponto e vírgula.{" "}
      ...
      {results !== null &&
        (results.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Informe ao menos um CPF para validar.
          </p>
        ) : (
                  ...
                  {result.valid ? (
                    result.region && (
                      <Badge variant="secondary">
                        {result.region.ordinal} RF · {result.region.states}
                      </Badge>
                    )
                  ) : (
